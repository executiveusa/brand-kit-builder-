...
export function installBrandbookTools(brandTools) {
  ...
  function render() {
    ...
    content.innerHTML = `<div class="brandbook-workspace"><aside class="book-navigation"><div class="book-progress"><div><strong>${approvedPages}/13</strong><small>${copy('approvedCount')}</small></div><div><strong>${includedAnnexes}/10</strong><small>${copy('annexCount')}</small></div></div><div><h3>${copy('core')}</h3><nav class="book-list">${items.filter((item) => item.type === 'section').map((item) => `<button type="button" class="${active === `section:${item.key}` ? 'is-active' : ''} ${item.done ? 'is-approved' : ''}" data-book-item="section:${item.key}" data-help="navigateBrandbookSection" aria-label="${esc(item.title)}"><span class="book-number">${item.number}</span><span>${esc(item.title)}</span><span class="book-status-dot" aria-hidden="true"></span></button>`).join('')}</nav></div><div><h3>${copy('annexes')}</h3><nav class="book-list">${items.filter((item) => item.type === 'annex').map((item) => `<button type="button" class="${active === `annex:${item.key}` ? 'is-active' : ''} ${item.done ? 'is-approved' : ''}" data-book-item="annex:${item.key}" data-help="navigateBrandbookAnnex" aria-label="${esc(item.title)}"><span class="book-number">${item.number}</span><span>${esc(item.title)}</span><span class="book-status-dot" aria-hidden="true"></span></button>`).join('')}</nav></div></aside><section class="book-editor"><header class="book-editor-header"><div><p class="eyebrow">${type === 'section' ? copy('page') : copy('annex')} ${type === 'section' ? meta.number : meta.letter}</p><h3>${esc(label(meta))}</h3><p>${copy('draftNotice')}</p></div><button id="toggle-book-preview" class="secondary-button" type="button" data-help="toggleBrandbookPreview" aria-label="${copy('preview')}">${copy('preview')}</button></header><form id="book-item-form"><label><span>${copy('content')}</span><textarea name="content" required>${esc(record.content)}</textarea></label>${type === 'section' ? `<label><span>${copy('sources')}</span><input type="text" name="source_refs" value="${esc((record.source_refs || []).join(', '))}" placeholder="${copy('sourceHint')}"></label><label class="book-approval"><input type="checkbox" name="approved" ${record.approved ? 'checked' : ''}><span>${copy('approved')}</span></label>` : `<label class="book-approval"><input type="checkbox" name="included" ${record.included ? 'checked' : ''}><span>${copy('included')}</span></label>`}<div class="book-notice ${complete ? 'is-complete' : ''}">${complete ? copy('complete') : copy('incomplete')}</div><div class="book-editor-actions"><div><button id="book-previous" class="secondary-button" type="button" data-help="brandbookPreviousPage" aria-label="${copy('previous')}" ${index === 0 ? 'disabled' : ''}>${copy('previous')}</button><button id="book-next" class="secondary-button" type="button" data-help="brandbookNextPage" aria-label="${copy('next')}" ${index === items.length - 1 ? 'disabled' : ''}>${copy('next')}</button></div><button class="primary-button" type="submit" data-help="saveBrandbookPage" aria-label="${copy('save')}">${copy('save')}</button></div></form><section id="book-preview-panel" class="book-preview-panel ${previewOpen ? '' : 'is-hidden'}"><header><strong>${copy('previewTitle')}</strong><button id="close-book-preview" class="text-button" type="button" data-help="closeBrandbookPreview" aria-label="${copy('closePreview')}">${copy('closePreview')}</button></header><iframe class="book-preview-frame" title="${copy('previewTitle')}" sandbox="allow-same-origin"></iframe></section></section></div>`;
