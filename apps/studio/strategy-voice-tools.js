...
function proofRow(item = {}, index = 0) {
  return `<tr data-proof-row><td><input name="proof_claim_${index}" value="${esc(item.claim)}" aria-label="${copy('claim')} ${index + 1}" required></td><td><input name="proof_source_${index}" value="${esc(item.source)}" aria-label="${copy('source')} ${index + 1}" placeholder="${copy('sourceHint')}" required></td><td><select name="proof_status_${index}" aria-label="${copy('status')} ${index + 1}"><option value="confirmed" ${selected(item.status, 'confirmed')}>${copy('confirmed')}</option><option value="provisional" ${selected(item.status, 'provisional')}>${copy('provisional')}</option><option value="blocked" ${selected(item.status, 'blocked')}>${copy('blocked')}</option></select></td><td><button class="text-button" type="button" data-remove-proof data-help="removeProofClaim" title="${copy('remove')}" aria-label="${copy('remove')}">${copy('remove')}</button></td></tr>`;
}
