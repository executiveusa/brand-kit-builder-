...
test('terminal approval is returned only after synchronized evidence and becomes stale after a change', async (t) => {
  ...
  await recordOwnerApproval(service.context, {
    ...
    action: 'export',
    approved_by: 'Bambu',
    confirmation_phrase: 'APPROVE EXPORT FOR HOST-BRAND',
    channel: 'terminal'
  });
  ...
  await service.invoke('sync-studio-project', { project_id: changed.project_id, project_snapshot: changed });
