...
|---|---|
| any human-facing design/communication/workflow | `adhd-elegant-simplicity/SKILL.md` |
| design-chat engineering workflow routing | `design-engineering-workflows/SKILL.md` |
| material UI/web/product visual direction or visual QA | `refero-design/SKILL.md` |
...
- Any human-facing surface: apply `adhd-elegant-simplicity` as an operating law, but do not let it override stronger security, proof, accessibility, owner-control, or stage rules.
- Design chat engineering request: route first through `design-engineering-workflows`; it chooses audit/reference-lock/brownfield/greenfield/visual-QA and loads only the specialists required.
- Material visual design/redesign/polish: add `refero-design` for research, reference lock, decision ledger and rendered drift validation.
...
- engineering/interface changes: `design-engineering-workflows` + `adhd-elegant-simplicity`; add `refero-design` when visual decisions are material; then `subtraction` + `completion-gates` before introducing new implementation.
...
- Refero-style research is a decision input, never permission to copy another product's visual work, assets, layouts or wording.
